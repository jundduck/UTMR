...
from navsim.agents.WoTE.utmr_selector import select_with_utmr
...
class WoTEModel(nn.Module):
    ...
    def select_best_trajectory(self, final_rewards, trajectory_anchors, batch_size):
        ...
        return self.select_trajectory_by_index(best_trajectory_idx, trajectory_anchors, batch_size)

    def select_trajectory_by_index(self, trajectory_idx, trajectory_anchors, batch_size):
        if trajectory_anchors.dim() == 4:
            batch_indices = torch.arange(batch_size, device=trajectory_idx.device)
            poses = trajectory_anchors[batch_indices, trajectory_idx]
        else:
            poses = trajectory_anchors[trajectory_idx]
        poses = poses.view(batch_size, 8, 3)  # Reshape to [batch_size, 8, 3]
        return poses

    def utmr_fine_reward_calculation(self, im_rewards, sim_rewards) -> torch.Tensor:
        assert len(sim_rewards) == 5, "Expected 5 metric rewards: S_NC, S_DAC, S_EP, S_TTC, S_COMFORT"
        s_nc, s_dac, s_ep, s_ttc, s_comfort = [reward.squeeze(-1).clamp_min(1e-6) for reward in sim_rewards]
        im_rewards = im_rewards.clamp_min(1e-12)
        return (
            getattr(self.config, "utmr_fine_im_weight", 0.0) * torch.log(im_rewards)
            + getattr(self.config, "utmr_fine_nc_weight", 1.0) * torch.log(s_nc)
            + getattr(self.config, "utmr_fine_dac_weight", 1.0) * torch.log(s_dac)
            + getattr(self.config, "utmr_fine_ep_weight", 0.5) * torch.log(s_ep)
            + getattr(self.config, "utmr_fine_ttc_weight", 1.0) * torch.log(s_ttc)
            + getattr(self.config, "utmr_fine_comfort_weight", 0.5) * torch.log(s_comfort)
        )
    ...
    def forward_test(self, features, targets=None) -> Dict[str, torch.Tensor]:
        ...
        use_utmr = getattr(self.config, "use_utmr", False)
        if use_utmr:
            s_nc, _, _, s_ttc, _ = [reward.squeeze(-1) for reward in sim_rewards]
            feasible_mask = (s_nc >= getattr(self.config, "utmr_min_nc", 0.5)) & (
                s_ttc >= getattr(self.config, "utmr_min_ttc_score", 0.5)
            )
            fine_rewards = self.utmr_fine_reward_calculation(im_rewards_softmax, sim_rewards)
            utmr = select_with_utmr(
                coarse_scores=final_rewards,
                fine_scores=fine_rewards,
                feasible_mask=feasible_mask,
                beta=getattr(self.config, "utmr_beta", 1.0),
                gamma_h=getattr(self.config, "utmr_gamma_h", 0.75),
                gamma_m=getattr(self.config, "utmr_gamma_m", 0.05),
                top_n=getattr(self.config, "utmr_top_n", 8),
                uniform_fine=getattr(self.config, "utmr_uniform_fine", False),
                fine_margin_min=getattr(self.config, "utmr_fine_margin_min", 0.0),
                max_coarse_drop=getattr(self.config, "utmr_max_coarse_drop", 1000000000.0),
            )
            poses = self.select_trajectory_by_index(utmr.selected_indices, trajectory_anchors, batch_size)
        else:
        ...
        results = {
            ...
            "sim_rewards": torch.stack([reward.squeeze(-1) for reward in sim_rewards], dim=1),
            }
        if utmr is not None:
            results.update(
                {
                    "utmr_selected_indices": utmr.selected_indices,
                    "utmr_baseline_indices": utmr.baseline_indices,
                    "utmr_entropy": utmr.entropy,
                    "utmr_margin": utmr.margin,
                    "utmr_triggered": utmr.triggered,
                    "utmr_feasible_count": utmr.feasible_count,
                    "utmr_feasible_mask": feasible_mask,
                    "utmr_fine_scores": fine_rewards,
                    "utmr_rerank_accepted": utmr.rerank_accepted,
                }
            )
        return results
