...
reranking을 구현했고**, 1000-scene subset에서 guarded safety UTMR가
baseline보다 높은 결과를 냈습니다.

| 실험 | Scene | 성공/실패 | Score | 의미 |
| --- | ---: | ---: | ---: | --- |
| WoTE baseline | 1000 | 1000 / 0 | 0.8638675087 | 기준선 |
| UTMR guarded safety | 1000 | 1000 / 0 | 0.8720460220 | 현재 best subset 결과 |

`UTMR guarded safety`는 baseline 후보를 무조건 바꾸지 않고, fine metric
score가 충분히 좋아지고 coarse score 손실이 제한될 때만 rerank를 받아들이는
방식입니다. 이 설정에서 1000 scene 기준 `rerank_accepted_pct = 9.5%`였습니다.
...
- 이 결과를 바탕으로 full `12146` guarded safety 실험을 시작했습니다.

### E. guarded safety full run

현재 문서 갱신 시점에는 full run이 진행 중입니다.
...
완료 후에는 `experiments/utmr/results/navsim_guarded_safety_full`의 로그를 분석해
full baseline과 full guarded safety UTMR score를 비교해야 합니다.
...
1. 진행 중인 guarded safety full run 완료 확인.
2. full baseline vs full guarded safety UTMR score 비교.
3. full에서도 개선되면 K=256 원본 WoTE anchor/cache 기준으로 반복.
4. `UTMR_FINE_MARGIN_MIN`과 `UTMR_MAX_COARSE_DROP` sensitivity를 1000-scene으로 추가 확인.
5. AWSIM/Autoware live batch 실행.
